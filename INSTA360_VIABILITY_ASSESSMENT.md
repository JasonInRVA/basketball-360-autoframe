...
1. Generate `.insprj` sidecar data from our model.
2. Pair sidecar + original source media.
...
- seamless parity with mobile-app transfer flows (`.insdata`) instead of Studio media-page projects (`.insprj`).
...
### 1) `.insprj` is an official Studio project artifact (media-page editing)
...
- **Strongly plausible:** model generates keyframe trajectory and writes `.insprj`, then user opens in Studio and exports.
- **Strongly plausible:** if needed, bypass Studio and render with official Media SDK (more engineering effort, but official route).

### What looks risky

- Assuming `.insprj` schema is stable, complete, and easy to synthesize without strict coupling to Studio version/media metadata.
- Assuming mobile (`.insdata`) and Studio (`.insprj`) are interchangeable.
...
1. Studio loads generated `.insprj` without error.
...
- There is strong evidence that `.insprj` is the correct Studio-side project artifact for media-page edits.
